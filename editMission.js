...
document.addEventListener('DOMContentLoaded', () => {
    ...
    updateAuthUINarBar();
    mission = getUserMissionData(currentMissionId);
});
...
async function getUserMissionData(missionId) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Return the data so you can use it to populate your UI
      return singleMissionData;
    } else {
      const errorText = await response.text();
      console.error(
        '--- Failed to fetch single mission ---',
        `Status: ${response.status}`,
        `Error: ${errorText}`
      );
      
      if (typeof missionDetailsContainer !== 'undefined' && missionDetailsContainer) {
        missionDetailsContainer.innerHTML = `<p>Error loading mission: ${response.status}</p>`;
      }
    }
  } catch (error) {
    console.error('--- Network Error Fetching Single Mission ---', error);
    if (typeof missionDetailsContainer !== 'undefined' && missionDetailsContainer) {
